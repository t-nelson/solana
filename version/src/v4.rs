...
pub enum Prerelease {
    Stable = Self::DISCRIMINANT_STABLE,
    ReleaseCandidate(u16) = Self::DISCRIMINANT_RELEASE_CANDIDATE,
    Beta(u16) = Self::DISCRIMINANT_BETA,
    Alpha(u16) = Self::DISCRIMINANT_ALPHA,
}

impl Prerelease {
    const DISCRIMINANT_STABLE: u16 = 0;
    const DISCRIMINANT_RELEASE_CANDIDATE: u16 = 1;
    const DISCRIMINANT_BETA: u16 = 2;
    const DISCRIMINANT_ALPHA: u16 = 3;
...
impl FromStr for Prerelease {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.is_empty() {
            ...
            assert_eq!(parts.next(), None, "Safety: splitn(2, ...)");
            let num =
...
impl PackedMinor {
    ...
    fn try_pack(
        ...
        let (prerelease_discriminant, patch) = match *prerelease {
            Prerelease::Stable => (Prerelease::DISCRIMINANT_STABLE, patch),
            Prerelease::ReleaseCandidate(rc) => (Prerelease::DISCRIMINANT_RELEASE_CANDIDATE, rc),
            Prerelease::Beta(beta) => (Prerelease::DISCRIMINANT_BETA, beta),
            Prerelease::Alpha(alpha) => (Prerelease::DISCRIMINANT_ALPHA, alpha),
        };
        let packed_minor = minor | prerelease_discriminant << Self::PRERELEASE_BITS_OFFSET;
    ...
    fn try_unpack(self, patch: u16) -> Result<(u16, u16, Prerelease), PackedMinorUnpackError> {
        ...
        let (patch, prerelease) = match prerelease_variant {
            Prerelease::DISCRIMINANT_STABLE => (patch, Prerelease::Stable),
            Prerelease::DISCRIMINANT_RELEASE_CANDIDATE => (0, Prerelease::ReleaseCandidate(patch)),
            Prerelease::DISCRIMINANT_BETA => (0, Prerelease::Beta(patch)),
            Prerelease::DISCRIMINANT_ALPHA => (0, Prerelease::Alpha(patch)),
...
mod tests {
    ...
    fn test_packed_minor_try_unpack() {
        ...
        if PackedMinor::PRERELEASE_BITS_OFFSET + PackedMinor::PRERELEASE_MASK_BITS < u16::BITS {
            assert_eq!(
                PackedMinor(0x8000).try_unpack(0),
                Err(PackedMinorUnpackError::ReservedBitsSet),
            )
        }
    }
